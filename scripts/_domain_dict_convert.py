...
Output: scripts/data/DomainWordsDict/{english_name}.json

Each JSON: {"sorted": true, "terms": [["term", weight], ...]}
Sorted by term (Python unicode order), compatible with bisect binary search.

Usage:
    python scripts/_domain_dict_convert.py
"""
...
SRC_DIR = os.path.join(SCRIPT_DIR, '..', 'thirdparty', 'DomainWordsDict')

# Simplified CJK collation: use term as-is (Python sorts by unicode codepoint,
# which for CJK characters yields a stable if not culturally-precise order).
